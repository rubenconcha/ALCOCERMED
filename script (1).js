...
async function loadVideoclases() {
    ...
    try {
        ...
        console.log('[Videoclases] cargados:', vcState.allVideos.length, 'videos');
        if (vcState.allVideos.length === 0) { setVcState('empty'); return; }
        populateVcMaterias();
        filterVideoclases();
    } catch (e) {
        setVcState('empty');
        console.error('[Videoclases] Error al cargar:', e.message, e);
        showToast('⚠ error cargando videoclases: ' + e.message, 'error');
    }
}
...
window.selectVcMateria = function (materia) {
    vcState.selectedMateria = materia ? materia.trim() : null;
...
window.filterVideoclases = function () {
    ...
    vcState.filteredVideos = vcState.allVideos.filter(function (v) {
        var vMateria = (campo(v, 'materia') || '').trim();
        var matchMat = !vcState.selectedMateria || vMateria === vcState.selectedMateria;
        var titulo = (campo(v, 'titulo') || '').toLowerCase();
        var desc = (campo(v, 'descripcion') || '').toLowerCase();
        var tema = (campo(v, 'tema') || '').toLowerCase();
        var matchQ = !q || titulo.includes(q) || desc.includes(q) || tema.includes(q);
...
function renderVcGrid() {
    ...
    vcState.filteredVideos.forEach(function (v) {
        var titulo    = v['TITULO']      || v['titulo']      || 'Sin título';
        var desc      = v['DESCRIPCION'] || v['descripcion'] || '';
        var materia   = v['MATERIA']     || v['materia']     || '';
        var tema      = v['TEMA']        || v['tema']        || '';
        var duracion  = v['DURACION']    || v['duracion']    || '';
        var miniatura = v['MINIATURA']   || v['miniatura']   || '';
        var urlVideo  = v['URL_VIDEO']   || v['url_video']   || v['URL'] || v['url'] || '';
...
window.openVcModal = function (v) {
    ...
    if (!modal) return;
    // Soporta columnas en MAYUSCULAS (TITULO, DESCRIPCION, MATERIA, URL_VIDEO)
    var titulo    = v['TITULO']      || v['titulo']      || '';
    var desc      = v['DESCRIPCION'] || v['descripcion'] || '';
    var materia   = v['MATERIA']     || v['materia']     || '';
    var urlVideo  = v['URL_VIDEO']   || v['url_video']   || v['URL'] || v['url'] || '';
    document.getElementById('vc-modal-title').textContent   = titulo;
    document.getElementById('vc-modal-desc').textContent    = desc;
    document.getElementById('vc-modal-materia').textContent = materia.toLowerCase();
    var embedUrl = getEmbedUrl(urlVideo);
